...
func init() {
	...
	// index used for fuzzy search benchmarking
	analysisQueue = index.NewAnalysisQueue(1)
	dictDocIndex, err = upside_down.NewUpsideDownCouch(gtreap.Name, nil, analysisQueue)
	if err != nil {
		panic(err)
	}
	err = dictDocIndex.Open()
	if err != nil {
		panic(err)
	}

	dictDocIndexDoc := document.NewDocument("1").
		AddField(document.NewTextField("name", []uint64{}, []byte("fun"))).
		AddField(document.NewTextFieldCustom("desc", []uint64{}, cracklib, twoDocIndexDescIndexingOptions, testAnalyzer)).
		AddField(document.NewTextFieldWithAnalyzer("street", []uint64{}, []byte("doesn't matter"), testAnalyzer))

	err = dictDocIndex.Update(dictDocIndexDoc)
	if err != nil {
		panic(err)
	}
}
